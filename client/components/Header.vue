    ...
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
      ...
      <div v-bind:class="{ hidden: !isMenuOpen, 'block md:flex': isMenuOpen }" class="w-full md:block md:w-auto" id="navbar-default">
        <ul class="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white">
          <li>
            <a href="#" class="block py-2 pl-3 pr-4 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0" aria-current="page">ВТБ ОНЛАЙН</a>
          </li>
          <li>
            <nuxt-link to="/map/offices" class="block py-2 pl-3 pr-4 text-blue-700 md:p-0">Карта</nuxt-link>
          </li>
          <li v-if="!isAuthenticated">
            <nuxt-link to="/authorization" class="block py-2 pl-3 pr-4 text-blue-700 md:p-0">Войти</nuxt-link>
          </li>
          <li v-if="!isAuthenticated">
            <nuxt-link to="/registration" class="block py-2 pl-3 pr-4 text-blue-700 md:p-0">Зарегистрироваться</nuxt-link>
          </li>
          <li v-if="isAuthenticated">
            <a href="#" @click="logout" class="block py-2 pl-3 pr-4 text-blue-700 md:p-0">Выйти</a>
          </li>
        </ul>
      </div>
    </div>
